...
HRESULT Quadtree::CreateTree(Camera* pMainCamera, Map* pMap, int maxDepth)
{
    ...
    _pMap = pMap;
    _maxDepth = maxDepth;
    _pRootNode = new Node(nullptr, _pMap, 0, pMap->_width - 1, pMap->_width * (pMap->_height - 1), pMap->_width * pMap->_height - 1, pMap->_height, pMap->_width);
...
HRESULT Quadtree::BuildTree(Node* pNode)
{
    ...
    pNode->CreateChildNode(pNode, _pMap, _pMap->_width, _pMap->_height);
