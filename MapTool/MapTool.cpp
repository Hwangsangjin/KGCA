...
BOOL CMapToolApp::InitInstance()
{
	...
	// MFC가 실행되면 레지스트리에 상태값을 저장하게 되는데, 화면이 갱신되면서 저장된 상태값을 읽어와서 계속 유지하기때문에 메인 프레임에 초기화만 시켜둔 값으론 조정이 안된다.
	// 레지스트리에 데이터를 저장하지 않도록 저장된 정보를 초기화 해주는 함수(CleanState)를 이용한다.
	...
	// 윈도우 핸들
	_sample.SetHandle(pView->m_hWnd);
