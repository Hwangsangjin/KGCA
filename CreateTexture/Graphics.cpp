...
{
    if (FAILED(InitDevice(hInstance, hWnd, width, height))) return E_FAIL;
    if (FAILED(InitShader())) return E_FAIL;
    if (FAILED(InitScene())) return E_FAIL;
...
HRESULT Graphics::InitDevice(HINSTANCE hInstance, HWND hWnd, UINT width, UINT height)
{
    ...
    if (FAILED(CreateSwapChain(hWnd, width, height))) return E_FAIL;
    if (FAILED(CreateRenderTargetView())) return E_FAIL;
    if (FAILED(CreateViewport(width, height)))return E_FAIL;
...
HRESULT Graphics::InitScene()
{
    if (FAILED(CreateVertexBuffer())) return E_FAIL;
    if (FAILED(CreateInputLayout())) return E_FAIL;
...
HRESULT Graphics::Render()
{
    ...
    _pImmediateContext->VSSetSamplers(0, 1, &_pSamplerState);
...
HRESULT Graphics::CreateSampler()
{
    ...
    sd.MinLOD = -FLT_MAX;
    sd.MaxLOD = FLT_MAX;
    hr = _pd3dDevice->CreateSamplerState(&sd, &_pSamplerState);
