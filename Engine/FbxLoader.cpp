...
{
    if (_pConstantBufferAnimBone)
    {
        _pConstantBufferAnimBone->Release();
        _pConstantBufferAnimBone = nullptr;
    }
...
HRESULT FbxLoader::SetDevice(ID3D11Device* pd3dDevice, ID3D11DeviceContext* pImmediateContext)
{
...
void FbxLoader::ParseMesh(FbxMesh* pFbxMesh, FbxSkinningObject3D* pObject)
{
    ...
    if (materialCount == 1)
    {
        pObject->texture_name_ = TextureManager::GetInstance()->GetSplitName(fileName);
    }
    ...
    if (materialCount > 1)
    {
        ...
        for (size_t i = 0; i < materialCount; i++)
        {
            pObject->_vbTextureList[i] = TextureManager::GetInstance()->GetSplitName(textureNames[i]);
        }
    }
    ...
    for (size_t polygon = 0; polygon < polygonCount; polygon++)
    {
        ...
        for (size_t face = 0; face < faceCount; face++)
        {
            ...
            for (size_t index = 0; index < 3; index++)
            {
                ...
                if (materialCount <= 1)
                {
                    pObject->vertices_.push_back(vertex);
...
void FbxLoader::UpdateAnimation(ID3D11DeviceContext* pImmediateContext)
{
    _animFrame = _animFrame + _animSpeed * _animScene.frameSpeed * _animInverse * Timer::GetInstance()->GetDeltaTime();
    ...
    for (size_t i = 0; i < _pObjects.size(); i++)
    {
        ...
        D3DXMatrixTranspose(&_cbDataBone.bone_matrix[i], &animMatrix);
        ...
        if (_pDrawObjects[iDraw]->_dxMatrixBindPoseMap.size())
        {
            for (size_t iBone = 0; iBone < _pObjects.size(); iBone++)
            {
                ...
                if (iter != _pDrawObjects[iDraw]->_dxMatrixBindPoseMap.end())
                {
                    ...
                    D3DXMatrixTranspose(&_cbDataBone.bone_matrix[iBone], &animMatrix);
                }
            }
...
void FbxLoader::UpdateSkeleton(ID3D11DeviceContext* pImmediateContext, float time, BoneBuffer& cbData)
{
    for (size_t i = 0; i < _pObjects.size(); i++)
    {
        DxMatrix animMatrix = _pObjects[i]->Interpolate(time, _animScene);
        cbData.bone_matrix[i] = animMatrix;
    }
}

void FbxLoader::UpdateSkinning(ID3D11DeviceContext* pImmediateContext, BoneBuffer& cbInputData, std::vector<BoneBuffer>& cbOutputData)
{
    // Skinning
    for (size_t iDraw = 0; iDraw < _pDrawObjects.size(); iDraw++)
    {
        if (_pDrawObjects[iDraw]->_dxMatrixBindPoseMap.size())
        {
            for (size_t iBone = 0; iBone < _pObjects.size(); iBone++)
            {
                ...
                {
                    DxMatrix bindMatrix = iter->second;
                    DxMatrix animMatrix = bindMatrix * cbInputData.bone_matrix[iBone];
                    D3DXMatrixTranspose(&cbOutputData[iDraw].bone_matrix[iBone], &animMatrix);
                }
            }
        }
    }
...
void FbxLoader::SetMatrix(DxMatrix* pWorld, DxMatrix* pView, DxMatrix* pProjection)
{
    if (pWorld)
    {
        _world = *pWorld;
