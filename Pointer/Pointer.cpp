...
void main()
{
	...
	*pPoint = 100;
	...
	// 멀티 바이트에서 _getche()==0 이라는 것은 확장키가 입력되었다는 것이다.
	...
	// 알파 또는 숫자키를 입력시 확장 키코드(널문자 추가)로 반환하는 
	// 문제가 시스템에 따라 발생한다.
	/*do {
	...
	{
		// 카운터, 주소, 값
		printf("\n*(iValue+%d): 0x%p=%d", iCnt,
			*(iValueArray + iCnt),
			*(*(iValueArray + iCnt) + 0));
		printf("\t\tiValue[%d]: 0x%p",
			iCnt, iValueArray[iCnt]);
	}
	printf("\n");

	int* pData = iValueArray[1];
	*pData++ = *pData + 100; // 130
	*pData++ = *pData + 100; // 140;

	for (int iCnt = 0; iCnt < 3; iCnt++)
	{
		printf("\n*(iValue+%d): %p", iCnt, *(iValueArray + iCnt));
		printf("\t\tiValue[%d]: %p", iCnt, iValueArray[iCnt]);
	}
	printf("\n");
	...
	for (int iCnt = 0; iCnt < 3; iCnt++)
	{
		...
		{
			printf("\n*(*(iValue+%d)+%d)  : %d",
				iCnt, jCnt, *(*(iValueArray + iCnt) + jCnt));
			printf("\t\t *(iValue[%d]+%d) : %d",
				iCnt, jCnt, *(iValueArray[iCnt] + jCnt));	// iValueArray[iCnt][jCnt]
		}
	}
...
void main()
{
	...
	*pPoint = 100;
	...
	std::cout << "아무키나 입력하면 프로그램 종료" << std::endl;
	_getch();
}
#endif
